-- Add "on the desk" widget fields to journal_settings.
-- Idempotent: safe to run multiple times and works whether or not
-- migration_004 has already been applied.
...
alter table public.journal_settings add column if not exists desk_label text default '';
alter table public.journal_settings add column if not exists desk_sublabel text default '';
